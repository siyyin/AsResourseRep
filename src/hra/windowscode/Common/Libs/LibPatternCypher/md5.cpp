...
static void __VSMD5_Transform (UINT4 *buf, UINT4 *in)
{
...
void __VSMD5Init (MD5_CTX *mdContext, unsigned long pseudoRandomNumber)
{
...
void __VSMD5Update(MD5_CTX* mdContext, const unsigned char* inBuf, unsigned int inLen)
{
    UINT4 in[16];
    int mdi = 0;
    ...
    while (inLen--)
    {
        ...
        if (mdi == 0x40)
        {
            ...
            __VSMD5_Transform (mdContext->buf, in);
            mdi = 0;
        }
    }
}

void __VSMD5Final (MD5_CTX *mdContext)
{
    UINT4 in[16];
    int mdi = 0;
    ...
    __VSMD5Update (mdContext, MD5_PADDING, padLen);
